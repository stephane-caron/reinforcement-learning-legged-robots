...
% Metropolis theme with Inria color scheme
...
% Citation in slide footnote
\newcommand\blfootcite[1]{%
    \invisible<1>{%
        \textcolor{white}{\footfullcite{#1}}%
    }%
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\title{\textsc{Reinforcement learning for legged robots}}

\author{{St\'{e}phane Caron}}

\date{\today}

\institute{D\'{e}partement d'informatique de l'ENS (\href{https://www.cnrs.fr}{CNRS}, \href{https://www.ens.psl.eu/en}{ENS-PSL}, \href{https://www.inria.fr/en}{Inria})}
...
\section{\textsc{Quick history of robot learning}}
...
\begin{frame}{2024: Bipedal locomotion}
    ...
    \end{center}
    \blfootnote{Video: \url{https://youtu.be/8pR1HE-wMHw}}
\end{frame}
...
\metroset{background=dark}
\section{\textsc{Intro to reinforcement learning}}
\metroset{background=light}
...
\begin{frame}{Partially observable Markov decision process (1/2)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Transitions:} only depend on current state and action (Markov property)
...
\begin{frame}{Partially observable Markov decision process (2/2)}
    ...
    \begin{table}
        \begin{tabular}{rlll}
            ...
            \hline
            \textbf{Transitions:} & $s_{t+1} = f(s_t, a_t)$ & $s_{t+1} \sim p(\cdot | s_t, a_t)$ & how the environment evolves \\
...
\begin{frame}{Value functions}
    \begin{columns}
        \begin{column}{0.6\textwidth}
            ...
            \begin{block}{On-policy value function}
                Expected return from $s$ following a given policy: $$V^\pi(s) = \mathbb{E}_{\tau \sim \pi}(R(\tau) | s_0 = s)$$
            \end{block}

            \begin{block}{Optimal value function}
                Best return we can expect from $s$: $$V^*(s) = \max_\pi \mathbb{E}_{\tau \sim \pi}(R(\tau) | s_0 = s)$$
            \end{block}
        \end{column}
        \begin{column}{0.39\textwidth}
            \begin{figure}
                \includegraphics[width=0.8\columnwidth]{genfig/labyrinth.pdf}
                \caption{
                    Labyrinth with discrete NSEW actions. Orange: distance to
                    the exit.
                }
            \end{figure}
        \end{column}
...
\begin{frame}{Bellman equation}
    ...
    \begin{block}{Bellman equation}
        ...
        \]
    \end{block}
    An optimal policy $\pi^*$ can thus be derived from an optimal value
    function $V^*$.

    This gives rise to algorithms like \href{https://en.wikipedia.org/wiki/Q-learning}{$Q$-learning} that search for
...
\begin{frame}{Components of an RL algorithm}
    ...
    \begin{itemize}
        \item \textbf{Policy:} compute the agent's action from an observation
        \item \textbf{Value function:} estimate the policy/best return from a state
        \item \textbf{Model:} an internal function approximating the (unknown) environment dynamics
    \end{itemize}
...
\begin{frame}{A taxonomy of RL algorithms}
    \begin{figure}
        ...
        \caption{
            We will focus on this branch in what follows.
        }
    \end{figure}
\end{frame}
...
\metroset{background=dark}
\section{\textsc{Policy optimization}}
\metroset{background=light}
...
\metroset{background=dark}
\section{\textsc{Application to robotics}}
\metroset{background=light}
...
\begin{frame}[standout]
    \textsc{Thank you for your attention!}\footnote{\color{white}%
        These slides have received feedback from E. Chane-Sane, T. Flayols, N.
        Perrin-Gilbert, R. P. Singh, P. Sou\`{e}res, and the 2023-2025 classes
        at ENS--PSL, Mines Paris--PSL and Master MVA. Thank you all!%
    }
\end{frame}
...
\section{\textsc{Bonus slides on PPO}}
